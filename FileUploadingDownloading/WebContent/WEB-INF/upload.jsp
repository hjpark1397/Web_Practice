...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    */
    ...
    String fileName1 = "";
    String fileName2 = "";
    String orgfileName1 = ""; //기존 파일 명
    String orgfileName2 = "";
    
    
    String uploadPath = request.getRealPath("upload");
    /*upload는 여기서 폴더명을 의미한다. 폴더의 경로를 구해오는 것*/
    ...
    try{
    		MultipartRequest multi = new MultipartRequest( /*MultipartRequest 인스턴스 생성 (cos.jar의 라이브러리)*/
    			...
    			/*
    			uploadPath = 파일을 저장할 디렉토리
    			10*1024 = 첨부파일 최대 용량 설정(bite)/10KB 용량 초과시 예외 발생
    		 	"utf-8" = 인코딩 방식
    		 	new DefaultFileRenamePolicy() = 중복 파일 처리(동일한 파일명이면 뒤에 숫자 등 붙여서 중복 방지)
    			*/
    			
    			id = multi.getParameter("id"); // form name = "id" 값
    			subject = multi.getParameter("subject"); // form name = "subject" 값
    			
    			/*
    			form의 <input type = "file"> name 값을 모를 경우 name을 구할 때 사용
    			...
    			*/
    			...
    			orgfileName1 = multi.getOriginalFileName("file1"); // name=file1의 업로드 된 원본 파일 이름을 구함(중복 처리 전 이름)
    			
    			fileName2 = multi.getFilesystemName("file2");
    			orgfileName2 = multi.getOriginalFileName("file2");
    			
    } catch (Exception e){
    	e.getStackTrace(); //업로드 종료
    }
    
    %>
    <!-- 업로드 된 파일을 확인하는 폼으로 이동! 위에서 구한 데이터를 hidden 방식으로 전송한다 -->
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>파일을 실제로 업로드할 로직 생성하기</title>
</head>
<body>
